...
const T={
en:{record:'Record',recording:'Tap to Stop',toastOk:'Seed planted!',toastDel:'Deleted',toastWelcome:'Welcome!',toastRec:'Recording...',toastTrans:'Transcribed!',toastErr:'Error',toastApi:'Enter valid API key',toastProf:'Select profession',toastNeed:'Need 3+ seeds',toastCopied:'Copied!',toastApiOk:'API works!',toastApiFail:'API invalid',discard:'Discard?',discardT:'Changes lost.',delIdea:'Delete?',delIdeaT:'Cannot undo.',all:'All',emptyT:'Your seeds await',emptyD:'Tap mic to plant your first idea',connEmpty:'No connections',connNeed:'Need 3+ seeds',catBusiness:'Business',catProduct:'Product',catContent:'Content',catPersonal:'Personal',catTech:'Tech',catAdd:'+ New',mStrategist:'Strategist',mStrategistR:'Market viability',mCritic:'Critic',mCriticR:'Finds flaws',mCreative:'Creative',mCreativeR:'Expands ideas',mPragmatic:'Pragmatic',mPragmaticR:'Execution',mInvestor:'Investor',mInvestorR:'ROI',mUser:'User',mUserR:'Customer view',colliderNeed:'Need 4+ seeds',colliderLimit:'Daily limit reached',collisionSaved:'Hybrid saved!',selectTwo:'Select two seeds',alreadyCollided:'Already crossed'},
es:{record:'Grabar',recording:'Detener',toastOk:'¡Semilla plantada!',toastDel:'Eliminada',toastWelcome:'¡Bienvenido!',toastRec:'Grabando...',toastTrans:'¡Transcrito!',toastErr:'Error',toastApi:'API key inválida',toastProf:'Selecciona profesión',toastNeed:'Necesitas 3+ semillas',toastCopied:'¡Copiado!',toastApiOk:'¡API funciona!',toastApiFail:'API inválida',discard:'¿Descartar?',discardT:'Se perderán cambios.',delIdea:'¿Eliminar?',delIdeaT:'No se puede deshacer.',all:'Todas',emptyT:'Tus semillas esperan',emptyD:'Toca mic para plantar tu primera semilla',connEmpty:'Sin conexiones',connNeed:'Necesitas 3+ semillas',catBusiness:'Negocio',catProduct:'Producto',catContent:'Contenido',catPersonal:'Personal',catTech:'Tech',catAdd:'+ Nueva',mStrategist:'Estratega',mStrategistR:'Viabilidad',mCritic:'Crítico',mCriticR:'Fallas',mCreative:'Creativo',mCreativeR:'Expande',mPragmatic:'Pragmático',mPragmaticR:'Ejecución',mInvestor:'Inversor',mInvestorR:'ROI',mUser:'Usuario',mUserR:'Cliente',colliderNeed:'Necesitas 4+ semillas',colliderLimit:'Límite diario',collisionSaved:'¡Híbrido guardado!',selectTwo:'Selecciona dos semillas',alreadyCollided:'Ya cruzadas'}
};

let S={user:null,ideas:[],cats:[{id:'business',emoji:'💼',color:'#00d4aa'},{id:'product',emoji:'📦',color:'#7c3aed'},{id:'content',emoji:'✍️',color:'#f59e0b'},{id:'personal',emoji:'🌿',color:'#ec4899'},{id:'tech',emoji:'🔧',color:'#3b82f6'}],conns:[],cur:null,selMentor:null,selCat:null,isRec:false,mr:null,chunks:[],recStart:0,recInt:null,step:1,searchQ:'',confirmCb:null,collisions:[],collidedPairs:[],collisionDate:null,collisionsToday:0,colliderTab:'discovered',selectedIdeas:[],pendingCollision:null};
...
const load=()=>{try{
// Migration from old IdeaForge storage
const oldData=localStorage.getItem('ideaforge-v2');
if(oldData&&!localStorage.getItem('spora-data')){
  localStorage.setItem('spora-data',oldData);
  localStorage.removeItem('ideaforge-v2');
  console.log('Migrated from IdeaForge to SPORA');
}
const d=localStorage.getItem('spora-data');if(d){const p=JSON.parse(d);S={...S,...p};S.collisions=S.collisions||[];S.collidedPairs=S.collidedPairs||[];S.selectedIdeas=[]}}catch(e){console.error('Load:',e)}};
...
const renderCollider=()=>{
  ...
  if(state==='empty'){
    ...
    $('colliderEmptyCount').textContent=`You have ${S.ideas.length} seed${S.ideas.length!==1?'s':''}.`;
  }else if(state==='limit'){
  ...
  try{
    const res=await fetch('https://api.openai.com/v1/chat/completions',{
      ...
      body:JSON.stringify({
        model:'gpt-4o',
        messages:[
          {role:'system',content:`You are a creative hybridizer. Cross-pollinate ideas to create unexpected offspring.\nRespond ONLY with JSON: {"title":"Hybrid Name (2-4 words)","synthesis":"One paragraph (<50 words) describing this new species of idea"}\nRespond in ${lang==='es'?'Spanish':'English'}.`},
          {role:'user',content:`Cross-pollinate these seeds:\n\nSEED A: "${ideaA.title}"\n${ideaA.content}\n\nSEED B: "${ideaB.title}"\n${ideaB.content}\n\nCreate a surprising hybrid.`}
        ],
        max_tokens:200
      })
    });
...
$('plantSeedsBtn').onclick=()=>goTo('capture');
